...
# When a wander attempt collides with a static obstacle, the *opposite* cardinal
# of the blocked heading is parked here so the next wander tick steps away
# from the obstacle instead of grinding into it again. Cleared on use.
...
func _wander_proximity_escape() -> Vector2:
	...
	if away.length() < 0.001:
		# Exact overlap (rare). No escape direction we can confidently pick —
		# defer to the next timer tick which will retry from scratch.
		return Vector2.ZERO
...
func _physics_process(delta):
	match movement_pattern:
		...
		_:
			...
			return
	move_and_slide()
	# If a wander step collided with a wall/object the NPC was still pushing
	# velocity into it every frame, locking the walk animation forever. Abort
	# the attempt so the next direction-timer tick can pick a fresh path that
	# steps AWAY from the obstacle.
...
func _abort_blocked_wander() -> void:
	# Snap the heading we were trying to walk into the obstacle to its cardinal
	# opposite, so the next pick steps away from whatever we just hit.
...
func _pick_wander_target():
	...
	# If we were just blocked, try the escape direction first. Consume the
	# hint regardless of whether the candidate is in-bounds — one attempt only.
